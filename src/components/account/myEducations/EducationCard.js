...
const EducationCard = ({ data }) => {
  ...
  const getCategoryStyle = (slug) => {
    // Slug bazen null gelebilir, kontrol edelim
    if (!slug) return "bg-gray-100 text-gray-800 border-gray-200";

    // Business için: Ciddi, Profesyonel (Mor/Indigo tonları)
    if (slug.includes("business")) {
      return "bg-indigo-600/90 text-white border-indigo-500 shadow-indigo-500/20";
    }

    // Daily için: Canlı, Ferah (Turkuaz/Mavi tonları)
    if (slug.includes("daily")) {
      return "bg-sky-500/90 text-white border-sky-400 shadow-sky-500/20";
    }

    // Kids veya diğerleri için (Opsiyonel): Turuncu/Sarı
    if (slug.includes("kids")) {
      return "bg-orange-500/90 text-white border-orange-400 shadow-orange-500/20";
    }

    // Varsayılan (Eşleşme olmazsa): Gri/Beyaz
  ...
  return (
    <div>
      ...
      {data?.map((item, i) => {
        ...
        const isCompleted = COMPLETED_STATUSES.includes(item.attendance_status);
...
`}
                >
                  {item?.course_session?.program?.category?.name}
                </div>
              </div>
              ...
              <div className="flex-1 p-6 flex flex-col gap-4">
                <div>
                  <h3 className="text-2xl font-bold text-black leading-tight pr-10">
                    {program?.title?.en || item?.course_session?.session_title}
                  </h3>
