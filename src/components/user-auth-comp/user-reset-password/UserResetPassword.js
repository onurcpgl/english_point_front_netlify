...
const ResetPasswordPage = () => {
  ...
  const checkResetPasswordStatus = async (token) => {
    if (!token) {
      setStatus({
        type: "error",
        message: "Geçersiz bağlantı. Lütfen tekrar deneyin.",
      });
      setLoading(false);
      return;
    }
    ...
    try {
      const result = await generalService.userCheckResetPasswordStatus(token);
      ...
      if (result.status === "success") {
        ...
        setStatus({
          type: "error",
          message: result.message || "Bu şifre sıfırlama bağlantısı geçersiz.",
        });
        setLoading(false);
      }
    } catch (err) {
  ...
  const handleSubmit = async (values, { setSubmitting, setStatus }) => {
    ...
    try {
      const res = await generalService.userResetPassword({
        token,
  ...
  useEffect(() => {
    if (token) checkResetPasswordStatus(token);
  }, [token]);
    ...
    <div className="min-h-screen bg-[#FFD207] flex items-center justify-center p-4">
      <div className="lg:w-1/2 flex flex-col items-center justify-center p-2 w-full max-w-md">
        <div className="text-start mb-5 w-full">
          <h2 className="text-3xl font-bold text-black mb-2">Şifre Sıfırla</h2>
        </div>
