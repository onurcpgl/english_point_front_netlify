  ...
  const handleLoginSubmit = async (values, { setSubmitting, setStatus }) => {
    try {
      ...
      const res = await signIn("credentials", {
        ...
        role: "instructor",
        redirect: false, // Otomatik yönlendirmeyi devre dışı bırakır
                  ...
                  {({ isSubmitting, status, errors, touched }) => (
                    <Form className="space-y-6">
                      ...
                      <button
                        type="submit"
                        disabled={isSubmitting}
                        className="w-full cursor-pointer bg-black hover:bg-white focus:bg-white focus:text-black hover:text-black text-white py-4 px-6 font-semibold focus:outline-none focus:ring-0 flex justify-center items-center gap-2 group transition-colors duration-300"
                      >
                        {isSubmitting ? (
                          <>
                            <Loader2 className="animate-spin" size={20} />
