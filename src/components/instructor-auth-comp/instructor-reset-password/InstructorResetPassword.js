...
const ResetPasswordPage = () => {
  ...
  const checkResetPasswordStatus = async (token) => {
    if (!token) {
      setStatus({ type: "error", message: "Invalid link. Please try again." });
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (result.status === "success") {
        ...
        setStatus({
          type: "error",
          message: result.message || "This password reset link is invalid.",
        });
        setLoading(false);
      }
    } catch (err) {
  ...
  // Token geçersiz veya kullanılmışsa
  ...
  // Token geçerli ise formu göster
  return (
