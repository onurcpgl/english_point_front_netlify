function MiniCourseSessionCard({ data }) {
  return (
    <>
      {data?.map((item, i) => (
        <article
          key={i}
          className="relative hover:scale-105 transition-all cursor-pointer bg-white rounded-xl shadow-md overflow-hidden flex "
        >
